...
def run_playbook():
    ...
    result = subprocess.run(['ansible-playbook', playbook, '-l', host], capture_output=True, text=True)
    logging.info(f'Playbook {playbook} executed by user {current_user.id} on host {host}.')
